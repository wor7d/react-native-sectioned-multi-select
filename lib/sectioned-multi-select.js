...
class SectionedMultiSelect extends PureComponent {
  ...
  _filterItems = searchTerm => {
    const { items, subKey, uniqueKey, displayKey } = this.props;
  ...
  _displaySelectedItems = () => {
    const { uniqueKey, selectedItems, colors } = this.props;
    ...
    return (
      <FlatList
        ...
        horizontal={true}
        renderItem={data => {
          let item = this._findItem(data.item);
  ...
  _renderItemFlatList = ({ item }) => {
    const { selectedItems, uniqueKey, subKey } = this.props;
